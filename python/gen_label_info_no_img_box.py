...
if __name__ == "__main__":
    ...
    dataset_path = '/mnt/fcav/datasets/panoptic/' # /home/trinhle/panoptic-toolbox/
    output_path = '/mnt/fcav/projects/bodypose2dsim/' # /home/trinhle/panoptic-toolbox/
    seq_name = '160422_ultimatum1' # 160422_haggling1
    kinect_img_path = dataset_path + seq_name + '/kinectImgs/'


    hd_skel_json_path = dataset_path + seq_name + '/hdPose3d_stage1_coco19/'
    label2_path = output_path + seq_name + '/training/label_2/'
    ...
    for (dirpath, dirnames, filenames) in os.walk(hd_skel_json_path):
        ...
        for filename in filenames:
            ...
            selUnivTime = psync['hd']['univ_time'][hd_index + 2 - 1] # -2 is some weired offset in synctables. -1 to account for 1-based
            ...
            print("hd_index:", hd_index, ", cindex=", cindex, ", dindex=", dindex, 'hd-depth diff={0: 0.4f}'.format(selUnivTime - k_depth['univ_time'][dindex]), 'hd-color diff= {0: 0.4f} \n'.format(selUnivTime - k_color['univ_time'][cindex] - 6.25))
            ...
            for id_ in range(len(bframe['bodies'])):
                ...
                rotation_y = -np.arctan2(orientation_cam_vec[2], orientation_cam_vec[0])
                

                label_str += 'Pedestrian 0.00 0 0 0 0 0 0 ' + str((height + 15)/100.0) + ' ' +  str((width+ 15)/100.0) + ' ' + str((length+ 15)/100.0) \
                                + ' ' + str(bottom_center_cam[0].item()/100.0) + ' ' + str(bottom_center_cam[1].item()/100.0) + ' ' + str(bottom_center_cam[2].item()/100.0) \
                                + ' ' + str(rotation_y.item()) + '\n'
            ...
            with open(label2_path + '/{0:02d}{1:02d}{2:08d}.txt'.format(50, idk, cindex), 'w') as lfile:
