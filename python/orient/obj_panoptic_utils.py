...
def read_labels(label_dir, img_idx, results=False):
    ...
    # Extract the list
    if os.stat(label_dir + "/%012d.txt" % img_idx).st_size == 0:
        return

    if results:
        p = np.loadtxt(label_dir + "/%012d.txt" % img_idx, delimiter=' ',
                       dtype=str,
...
def get_lidar_point_cloud(img_idx, calib_dir, velo_dir,
    ...
    frame_calib = calib_panoptic_utils.read_calibration(calib_dir, img_idx)
    # x, y, z, i = calib_panoptic_utils.read_lidar(velo_dir=velo_dir, img_idx=img_idx)
    x, y, z = calib_panoptic_utils.read_lidar_xyz(velo_dir=velo_dir, img_idx=img_idx)

    # Calculate the point cloud
    pts = np.vstack((x, y, z)).T
    # Since we already translate pointclouds in to camera frame, we don't need to calibrate again.
    # pts = calib_panoptic_utils.lidar_to_cam_frame(pts, frame_calib)
    ...
    if not im_size:
        ...
        pts = pts[pts[:, 2] > 0]
        point_cloud = pts.T
        distortion = frame_calib.Kd_11

        # Project to image frame
        point_in_im = calib_panoptic_utils.project_to_image(point_cloud, p=frame_calib.HD_11, dist=distortion).T
...
def project_points_on_2D_image(img_idx, calib_dir, im_size, pts):
    """ Project lidar points to the image. lidar points should be in camera
    coordinates already.

    :param img_idx: image index
    :param calib_dir: directory with calibration files
    :param im_size: 2 x 1 list containing the size of the image
                    to filter the point cloud [w, h]
    :param pts: Input pointclouds ((N x 4), where 4 = [x,y,z,i])

    :return: (2, N) points on the image [[x,...][y,...]]
    """
    ...
    frame_calib = calib_panoptic_utils.read_calibration(calib_dir, img_idx)
    # Project to image frame
    point_in_im = calib_panoptic_utils.project_to_image(point_cloud, p=frame_calib.HD_11, dist=frame_calib.Kd_11).T
    ...
    return point_in_im[image_filter].T
...
def get_road_plane(img_idx, planes_dir):
    ...
    plane_file = planes_dir + '/%012d.txt' % img_idx
...
def project_box3d_to_image(corners_3d, p, distortion=None):
    ...
    return calib_panoptic_utils.project_to_image(corners_3d, p, dist=distortion), face_idx


def compute_orientation_3d(obj, p, distortion=None):
    ...
    return calib_panoptic_utils.project_to_image(orientation3d, p, dist=distortion)
...
def get_point_filter(point_cloud, extents, ground_plane=None, offset_dist=2.0):
    ...
    if ground_plane is not None:
        ...
        # print('extents_filter = ', extents_filter)
        # print('plane_filter = ', plane_filter)
