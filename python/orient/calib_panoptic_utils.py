...
def read_calibration(calib_dir, img_idx):
    ...
    data_file = open(calib_dir + "/%012d.txt" % img_idx, 'r')
    ...
    frame_calibration_info.HD_11 = p_all[0]

    # Read in rectification matrix
    tr_rect = data[1]
    tr_rect = tr_rect[1:]
    tr_rect = [float(tr_rect[i]) for i in range(len(tr_rect))]
    frame_calibration_info.Kd_11 = np.reshape(tr_rect, (1, 5))

    # Read in velodyne to cam matrix
    tr_v2c = data[2]
    tr_v2c = tr_v2c[1:]
    tr_v2c = [float(tr_v2c[i]) for i in range(len(tr_v2c))]
    frame_calibration_info.Tr_pan_to_cam_11 = np.reshape(tr_v2c, (3, 4))
...
def read_disparity(disp_dir, img_idx):
    """Reads in Disparity file from Kitti Dataset.
        ...
        """
    disp_path = disp_dir + "/%012d_left_disparity.png" % img_idx
...
def read_lidar(velo_dir, img_idx):
    """Reads in PointCloud from Kitti Dataset.
        ...
        """
    velo_file = velo_dir + "/%012d.ply" % img_idx

    try:
        # print('velo_file = ', velo_file)
        ptclouds = PyntCloud.from_file(velo_file)  # ptclouds is Panda DataFrame
        # print('ptclouds = ', ptclouds.points)
    except Exception:
        return []
    ...
    red = ptclouds.points["red"].values
    green = ptclouds.points["green"].values
    blue = ptclouds.points["blue"].values
    # print("red = ", red)
    # print("green = ", green)
    # print("blue = ", blue)
    i = np.mean([red, green, blue], axis=0)
    # print("i = ", i)

    return x, y, z, i


def read_lidar_xyz(velo_dir, img_idx):
    """Reads in PointCloud from Kitti Dataset.
