% A program to generate LIDAR-resembled pointcloud from 1 specified Kinect and  
% project on a camera above and close to Kinect on specified frame.
% 
% Note: the output pointcloud is saved as 2 ply files: one w.r.t Kinect
% local and one w.r.t camera coordinate system.
%
% Note2: the camera image of specified frame is saved.
%
% Note3: Frame indices of output ply files are consistent to HD frames of the
% Panoptic Studio. That is ptcloud_000000001.ply is the first frame of HD videos
%
% Note4: This code assumes that your matalb has Computer Vision toolbox to
...
% gen_ptcloud_persons_camera_view('/home/trinhle/panoptic-toolbox/', '/home/trinhle/panoptic-toolbox/', '160422_haggling1', 2, 6745, 6745)
% 50_01_00006745

function gen_ptcloud_persons_camera_view(dataset_path, output_path, seqName, seqUid, seqBegin, seqEnd) 

%dataset_path = '/mnt/fcav/datasets/panoptic/' %Put your root path where sequence folders are locates % was: /home/trinhle/panoptic-toolbox/
%output_path = '/mnt/fcav/projects/bodypose2dsim/';
%seqName = '160226_haggling1'  %Put your target sequence name here
%seqUid = 1
%kcolor_index_list= 1:12019; % Target frames you want to export ply files. Note: not all HD frames will have corresponding Kinect frames.
...
plyOutputDir=sprintf('%s/%s/training/ptclouds_dome_floor_filtered_kinect_view_01',output_path,seqName); % was root_path
...
lidarUpperThreshold = 97; % Adjust this (0.5cm ~ 7cm), if floor points are not succesfully removed
                            % Icreasing this may remove feet of people
lidarLowerThreshold = 96;                    
...
%         end
        ...
        rgbFileName = sprintf('%s/50_%02d/50_%02d_%08d.jpg',kinectImgDir,idk,idk,cindex); % rgbFileName: 50_01/50_01_00000001 for cindex = 00000001 so cindex represents an image frame?
        
        % Depth image for selected kinect
        depthFileName = sprintf('%s/KINECTNODE%d/depthdata.dat',kinectDepthDir,idk); % depthFileName: KINECTNODE1
        try
        ...
        camCalibData = kinect_calibration.sensors{idk}; % kinect_calibration holds kcalibration_%s.json => camCalibData holds Calibration Data for selected kinect
        ...
        % point3d = Nx3 array with X,Y,Z coordinates w.r.t DEPTH camera frame
        % point2d_incolor: Nx2 projected points. perhaps point2d_incolor just contains X, Y on IMAGE frame             
        [point3d, point2d_incolor] = unprojectDepth_release(depthim, camCalibData, true); % depthim is reversed so point3d gots reversed as well.
        ...
        validMask = validMask(:) &  ~(point3d(:,1)==0); % Check X!=0
        nonValidPixIdx = find(validMask(:)==0);
        point3d(nonValidPixIdx,:) = nan;
        point2d_incolor(nonValidPixIdx,:) = nan; % Same indices even though different frames
        ...
        if vis_output
            ...
            title('Pointcloud after removing walls');
            view(2);
            waitforbuttonpress;
        end
        ...
        if bRemoveFloor
            % Delete floor points
            % Crop floor 
            floorPtIdx =(find(point3d_pan(:,2)>-floorHeightThreshold));      %Up-direction => negative Y axis
            ...
            point3d_pan(outliers,:) =[];
            colorsv(outliers,:) =[];
            
        end
        
        if vis_output
            figure; plot3(point3d_pan(:,1),point3d_pan(:,2),point3d_pan(:,3),'.'); axis equal;  %Plot raw 3D points
            title('Pointcloud after removing floors in Panoptic frame.');
            view(2);
            waitforbuttonpress;
        end
        ...
        point3d_cam = bsxfun(@plus, panoptic_calibData.R * point3d_pan', panoptic_calibData.t(:))' ./ 100.0;
        [point2d_color] = PoseProject2D(point3d_pan, panoptic_calibData, 1);
%         size(point3d_cam)
        %% Run DBSCAN Clustering Algorithm on point3d_cam
%         epsilon=0.5;
%         MinPts=10;
%         IDX=DBSCAN(point3d_cam,epsilon,MinPts);
%         %% Plot Results
%         PlotClusterinResult(point3d_cam, IDX);
%         title(['DBSCAN Clustering (\epsilon = ' num2str(epsilon) ', MinPts = ' num2str(MinPts) ')']);
%         epsilon=0.5;
%         minPoints=10;
%         point3d_cam_dbscan = dbscan(point3d_cam, epsilon, minPoints);
%         PlotClusterinResult(point3d_cam, point3d_cam_dbscan(:, :, :, 3));
        
        %% Save pointcloud point3d_incam with colorsv here and Save rgb image
        out_pc_color = pointCloud(point3d_cam);
        out_pc_color.Color = uint8(round(colorsv*255));
        ...
        if vis_output
            figure; scatter3(point3d_cam(:,1),point3d_cam(:,2),point3d_cam(:,3),'.'); axis equal;
            title('Point Cloud in Cam frame.');
            waitforbuttonpress;
            
        end
        
        if vis_output
            figure; scatter(point2d_color(:,1),point2d_color(:,2), 1,colorsv);  axis equal; % Each point cloud is a circle with size = 1
            title('Pt cloud in Image Frame');
            view(2);
            waitforbuttonpress;
        end
        ...
        if vis_output
            ...
            plot(point2d_color(:,1),point2d_color(:,2),'.');
            axis equal;
