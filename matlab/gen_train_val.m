% A program to generate 2D synthethic LIDAR pointcloud from 1 specified Kinect and 
% Kinect images into lidar_ptclouds and rgb_images directory.
% 
...
% lidarUpperThreshold and lidarLowerThreshold (in centimeters) can be set to choose where the 2D slice is above the ground. Default is between 96 and 97.
%
% Note3:
...
% Output Path Setting %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (output_path)/(seqName)/lidar_ptclouds
...
function gen_train_val(dataset_path, output_path, seqName, seqUid, seqBegin, seqEnd) 
...
plyOutputDir=sprintf('%s/%s/training/lidar_ptclouds',output_path,seqName); % was root_path
...
for cindex = kcolor_index_list
    ...
    for idk = kidx  %Iterating 10 kinects. Change this if you want a subpart
        ...
        lidarPtIdx =(find(point3d_pan(:,2) < -lidarUpperThreshold | point3d_pan(:,2) > -lidarLowerThreshold));      %Up-direction => negative Y axis
        point3d_pan(lidarPtIdx,:) =[];
        colorsv(lidarPtIdx,:) =[];
        ...
        [point2d_lidar_color] = PoseProject2D(point3d_pan, panoptic_calibData, 1);
        
        
        %% Save pointcloud point3d_incam with colorsv here and Save rgb image
        try
            rgb_filename = sprintf('%s/%02d%02d%02d%06d.jpg',rgbImageDir, 50, idk, seqUid, cindex);
            imwrite(rgbim, rgb_filename,'jpg');
        catch me
            fprintf('Cannot save Kinect image: %08d\n', cindex);
            continue
        end
        
        out_pc_color = pointCloud(point3d_lidar_cam);
        ...
        if vis_output
            figure; scatter3(point3d_lidar_cam(:,1),point3d_lidar_cam(:,2),point3d_lidar_cam(:,3),'.'); axis equal;
        ...
        if vis_output
            figure; scatter(point2d_lidar_color(:,1),point2d_lidar_color(:,2), 1,colorsv);  axis equal; % Each point cloud is a circle with size = 1
